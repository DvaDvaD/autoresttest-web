  ...
  beforeEach(() => {
    ...
    describe('GET', () => {
      ...
      it('should return list of jobs for authenticated user', async () => {
        ...
        jest.mocked(prisma.job.findMany).mockResolvedValue(mockJobs as any);
      ...
      it('should handle errors', async () => {
          jest.mocked(auth).mockRejectedValue(new Error('DB Error'));
          const response = await GET();
    ...
    describe('POST', () => {
      ...
      it('should authenticate with Bearer token if Clerk auth fails', async () => {
         jest.mocked(auth).mockResolvedValue({ userId: null } as any);
         jest.mocked(prisma.userApiKey.findUnique).mockResolvedValue({ userId: 'user_from_key' } as any);
         ...
         jest.mocked(prisma.job.create).mockResolvedValue({ id: 'job_new' } as any);
      ...
      it('should return 401 if API key is invalid', async () => {
         jest.mocked(auth).mockResolvedValue({ userId: null } as any);
         jest.mocked(prisma.userApiKey.findUnique).mockResolvedValue(null);
      ...
      it('should update job status to failed if trigger fails', async () => {
          jest.mocked(auth).mockResolvedValue({ userId: 'user_123' } as any);
          jest.mocked(prisma.job.create).mockResolvedValue({ id: 'job_fail' } as any);
          jest.mocked(tasks.trigger).mockRejectedValue(new Error('Trigger failed'));  
        const req = new Request('http://localhost/api/v1/jobs', {
          method: 'POST',
          body: JSON.stringify(validBody),
        });
