...
export function OperationStatusCodesChart({ url }: { url: string }) {
    const { data, isLoading, isError, error } = useQuery<Record<string, Record<string, number>>> ({
    ...
    const { chartData, chartConfig, allKeys } = useMemo(() => {
        if (!data) return { chartData: [], chartConfig: {} as ChartConfig, allKeys: [] };
        
        const keys = new Set<string>();
        const transformedData = Object.entries(data).map(([operationName, codes]) => {
        ...
        Object.values(groups).forEach(group => {
            const baseColorKey = group[0].charAt(0) in BASE_OKLCH_COLORS ? group[0].charAt(0) : 'default';
    ...
    if (isError) {
        return (
            <Alert variant="destructive">
                ...
                <AlertDescription>Failed to load chart data: {error.message}</AlertDescription>
            </Alert>
        );
    }
